...
set DOCKERFILE_DIR=C:\Users\ACER_COREI5\Documents\GitHub\Node
...
docker run -d -p %PUERTO_MAPEADO% --name %CONTENEDOR% -e NODE_NAME=NODO%CONTADOR% -e NODE_SERVICE_IP=172.17.0.%CONTADOR% -e NODE_SERVICE_PORT=%PORT% -e NODE_VERSION=18.19.1 -e PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin -e YARN_VERSION=1.22.19 %IMAGEN%
